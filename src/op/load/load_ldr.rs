...
/// Execute the LDR instruction (base register + offset load).
///
/// Loads the memory word at `base + offset6` into the destination register and
...
pub fn load_register(vm: &mut VM, instr: u16) {
    ...
    vm.set_reg(dr.into(), vm.read_mem(md.wrapping_add(offset)));
    update_flags(dr, vm);
}
